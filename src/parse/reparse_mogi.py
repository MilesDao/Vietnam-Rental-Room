"""Rebuild data/interim/parsed_mogi.parquet from the retained raw HTML.

The crawler keeps every detail page gzipped under data/raw/html/mogi/<date>/,
independently of the Parquet, precisely so parsed output is disposable. Use
this when:

- a parser bug or a mogi markup change means the Parquet is wrong -- re-parse
  instead of re-crawling (docs/mogi_scraping.md section 3);
...
Usage:
    ...
    python -m src.parse.reparse_mogi --dry-run    # just report what it'd do
"""
...
DB_PATH = REPO_ROOT / "data" / "mogi_seen_urls.db"
OUT_PATH = REPO_ROOT / "data" / "interim" / "parsed_mogi.parquet"

log = logging.getLogger("reparse_mogi")
...
def main() -> None:
    ...
    for path in files:
        ...
        if url is None:
            ...
            missing_url += 1
            url = f"https://mogi.vn/-id{listing_id.removeprefix('mogi_')}"
        try:
